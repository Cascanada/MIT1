...
def bipartiteGraphColor(graph, start, coloring, color):
    # Is starting note in the keys in the graph?
    if start not in graph:
        return False, {}

    # Is there already a colour at the node? If not, colour the node!
    # If yes 1) is the current colour the colour I want to colour it? If yes, good (termination condition).
    # If not it fails!
